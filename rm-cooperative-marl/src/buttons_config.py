...
def buttons_config(num_times, num_agents):
    ...
    joint_rm_file = os.path.join('rm-cooperative-marl', 'experiments', 'buttons', 'team_buttons_rm.txt')

    local_rm_files = []
    for i in range(num_agents):
        local_rm_string = os.path.join('rm-cooperative-marl', 'experiments', 'buttons', 'buttons_rm_agent_{}.txt'.format(i+1))
