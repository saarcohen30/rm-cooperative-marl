...
def plot_multi_agent_results(tester, num_agents):
    ...
    plt.plot(steps, prc_50, color='blue')
    plt.plot(steps, prc_75, alpha=0)
    plt.grid()
    plt.fill_between(steps, prc_50, prc_25, color='blue', alpha=0.25)
    plt.fill_between(steps, prc_50, prc_75, color='blue', alpha=0.25)
    plt.ylabel('Testing Steps to Task Completion', fontsize=15)
    plt.xlabel('Training Steps', fontsize=15)
    plt.locator_params(axis='x', nbins=5)
    
    plt.show()
    fig.savefig('iql'+ str(num_agents) + '.png')